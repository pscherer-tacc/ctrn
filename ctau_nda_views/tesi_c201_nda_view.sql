--- tesi_c201 nda view utilizes the tesic_union_helper view to provide both tesic baseline and tesic_followup
--- data for CTAU participants only. The resulting dataset includes values for curation which must be removed
--- prior to sharing. The variable names and selectors are harmonized to the NDA tesi_c201 template.
--- This view was modelled after the VT tesic view. "crit_a1" variables were added for 2_2 thru 2_5, 3_1 thru 3_3, 4_4 thru 4_3 , and 5. 

select
    dem.dem_guid as subjectkey
    ,sa1.subject_id as src_subject_id
	,tesic_u.source_subject_id -- only for validation; DELETE before submission
	,case
