...
left join (
    ...
    on mini_duplicates.source_subject_id = mini_view.source_subject_id
    and mini_duplicates.event_name = mini_view.event_name
...
create or replace view mini_ctrn_view
as
select
    ...
    case															-- Renaming events as visits to facilitate sequencing as per 4/30/2026 request by Jeff
	    when mini.event_name like 'baseline%' then '00_baseline'      
		when mini.event_name like 'one_month%' then '01_one_month'
		when mini.event_name like 'six_month%' then '06_six_month'
		when mini.event_name like 'one_year%' then '12_month'
