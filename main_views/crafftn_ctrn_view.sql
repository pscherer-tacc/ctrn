...
select
	case
		when crafftn_duplicates.source_subject_id is not null then 'YES'
		else 'NO'
	end as potential_duplicate,
	crafftn_view.* 
from crafftn_ctrn_view crafftn_view
left join (
	select source_subject_id, event_name
	from crafftn_ctrn_view
	group by source_subject_id, event_name
	having count(*) > 1
) crafftn_duplicates
	on crafftn_duplicates.source_subject_id = crafftn_view.source_subject_id
	and crafftn_duplicates.event_name = crafftn_view.event_name
...
order by
	...
	crafftn_view.source_subject_id,
	crafftn_view.event_name
;



-- The body of the view
create or replace view crafftn_ctrn_view
as
select
